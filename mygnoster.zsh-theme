...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    vcs_info
    LEFTPROMPT="$LEFTPROMPT${ref/refs\/heads\//$PL_BRANCH_CHAR }${vcs_info_msg_0_%% }${mode}"
  fi
}
...
prompt_hg() {
  ...
  if $(hg id >/dev/null 2>&1); then
    if $(hg prompt >/dev/null 2>&1); then
      ...
      LEFTPROMPT="$LEFTPROMPT$(hg prompt "☿ {rev}@{branch}") $st"
    else
      st=""
      ...
      LEFTPROMPT="$LEFTPROMPT☿ $rev@$branch $st"
    fi
  fi
}
...
prompt_date() {
  RIGHTPROMPT="$RIGHTPROMPT%{%F{black}%}%{%K{black}%f%}$(date +%H:%M:%S)%{%k%}"
}
...
build_prompt() {
  ...
  (( spacing_width = $COLUMNS - ${#${(S%%)RIGHTPROMPT//$~zero/}} ))
  for i in {1..$spacing_width}; do
    spacing="${spacing}─"
  done
  spacing="%{%F{black}%}${spacing}"

  echo -n "${spacing}${RIGHTPROMPT}"
  echo "\r${LEFTPROMPT}"
  echo -n "➜"
}
